...
fn init_clients(
    ...
    clients.iter_mut()
    {
        ...
        *game_mode = GameMode::Adventure;
        ...
        match write_to_file(format!("A player has joined the server!"), "config/logs.txt") {
            Ok(()) => {},
            n => println!("An error occured when tried to write logs (join): {n:?}")
        }
    ...
    for (mut client, mut pos, mut look, mut state, mut layer) in clients.iter_mut() {
        ...
        if out_of_bounds || state.is_added() {
            if out_of_bounds && !state.is_added() {
                ...
                match write_to_file(format!("A player fell into the void with the score {}.", state.score), "config/logs.txt") {
                    Ok(()) => {},
                    n => println!("An error occured when tried to write logs (death): {n:?}")
                }
            }

            // Init chunks.
            for pos in ChunkView::new(ChunkPos{
                x: START_POS.x.div_euclid(16),
                z: START_POS.z.div_euclid(16),
            }, VIEW_DIST).iter() {
...
fn manage_chunks(mut clients: Query<(&Position, &OldPosition, &mut ChunkLayer), With<Client>>) {
    for (pos, old_pos, mut layer) in &mut clients {
        let old_view = ChunkView::new(ChunkPos{
            x: old_pos.get().x.div_euclid(16f64) as i32,
            z: old_pos.get().z.div_euclid(16f64) as i32,
        }, VIEW_DIST);
        let view = ChunkView::new(ChunkPos{
            x: pos.get().x.div_euclid(16f64) as i32,
            z: pos.get().z.div_euclid(16f64) as i32,
        }, VIEW_DIST);
